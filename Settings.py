...
Path = 'C:\\Users\\admin\\Desktop\\VagrantBoxes\\LSDTopoTools\\Topographic_projects\\lidar\\Luquillo\\Hillslope_properties\\' #Must be wrapped in quotes with a trailing slash eg 'home/user/data/'
Prefix = 'RBClip' #Must be wrapped in quotes and match the prefix used in ER_STAR.cpp
Order = 3 #Basin order used in ER_STAR.cpp to extract the drainage basins. eg 1,2,5

#Options to select data to be plotted
RawFlag = 0 #Use 1 to plot the raw data and 0 to not plot it.
DensityFlag = 2 #Use 0 to not plot the raw data as a density plot and 1 to plot
...
#Plot style options
ErrorBarFlag = True #True to plot errorbars on datapoints, False to exclude them.
